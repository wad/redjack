...
public class PlayStrategyHiLoRealisticHardHandsTest extends HiLoRealisticStrategyTestHelper {
        ...
        for (Value upcardValue : Value.values()) {
            switch (upcardValue) {
                ...
                case Four:
                    ...
                    assertEquals(BlackjackPlay.Stand, compute(rc(-2), c5(), c7(), c(upcardValue))); // don't deviate
                    assertEquals(BlackjackPlay.Stand, compute(rc(-1), c5(), c7(), c(upcardValue))); // don't deviate
                ...
                case Five:
                    ...
                    assertEquals(BlackjackPlay.Stand, compute(tc(-3), c5(), c7(), c(upcardValue))); // don't deviate
                    assertEquals(BlackjackPlay.Stand, compute(tc(-2), c5(), c7(), c(upcardValue))); // don't deviate
        ...
        for (Value upcardValue : Value.values()) {
            switch (upcardValue) {
                case Two:
                    ...
                    assertEquals(BlackjackPlay.Stand, compute(rc(-2), c6(), c7(), c(upcardValue))); // don't deviate
                    assertEquals(BlackjackPlay.Stand, compute(rc(-1), c6(), c7(), c(upcardValue))); // don't deviate
                ...
                case Three:
                    ...
                    assertEquals(BlackjackPlay.Stand, compute(tc(-3), c6(), c7(), c(upcardValue))); // don't deviate
                    assertEquals(BlackjackPlay.Stand, compute(tc(-2), c6(), c7(), c(upcardValue))); // don't deviate
        ...
        for (Value upcardValue : Value.values()) {
            switch (upcardValue) {
                ...
                case King:
                    ...
                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCanSurrender, tc(2), false, c6(), c8(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCanSurrender, tc(3), false, c6(), c8(), c(upcardValue))); // don't deviate
                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCanSurrender, tc(4), false, c6(), c8(), c(upcardValue))); // don't deviate

                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCanSurrender, tc(2), true, c6(), c8(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, tc(3), true, c6(), c8(), c(upcardValue))); // deviate
                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, tc(4), true, c6(), c8(), c(upcardValue))); // deviate
                    break;
                case Ace:
                    assertEquals(BlackjackPlay.Hit, compute(c4(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Hit, compute(c5(), c9(), c(upcardValue)));
        ...
        for (Value upcardValue : Value.values()) {
            switch (upcardValue) {
                ...
                case Nine:
                    ...
                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCanSurrender, tc(1), false, c7(), c8(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCanSurrender, tc(2), false, c7(), c8(), c(upcardValue))); // don't deviate
                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCanSurrender, tc(3), false, c7(), c8(), c(upcardValue))); // don't deviate

                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCanSurrender, tc(1), true, c7(), c8(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, tc(2), true, c7(), c8(), c(upcardValue))); // deviate
                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, tc(3), true, c7(), c8(), c(upcardValue))); // deviate
                    break;
                case Ten:
                case Jack:
                case Queen:
                case King:
                    ...
                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCannotSurrender, tc(3), false, c5(), c4(), c6(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCannotSurrender, tc(4), false, c5(), c4(), c6(), c(upcardValue))); // don't deviate
                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCannotSurrender, tc(5), false, c5(), c4(), c6(), c(upcardValue))); // don't deviate

                    assertEquals(BlackjackPlay.Hit, compute(tableRulesCannotSurrender, tc(3), true, c5(), c4(), c6(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, tc(4), true, c5(), c4(), c6(), c(upcardValue))); // deviate
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, tc(5), true, c5(), c4(), c6(), c(upcardValue))); // deviate
        ...
        for (Value upcardValue : Value.values()) {
            switch (upcardValue) {
                ...
                case King:
                    ...
                    // the random check comes back false

                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, rc(-1), false, c6(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, rc(0), false, c6(), cT(), c(upcardValue))); // don't deviate
                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, rc(1), false, c6(), cT(), c(upcardValue))); // don't deviate
                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, rc(-1), false, c7(), c9(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, rc(0), false, c7(), c9(), c(upcardValue))); // don't deviate
                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, rc(1), false, c7(), c9(), c(upcardValue))); // don't deviate

                    // handle case where can't surrender due to more than 2 cards in hand
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCanSurrender, rc(-1), false, c2(), c4(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCanSurrender, rc(0), false, c2(), c4(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCanSurrender, rc(1), false, c2(), c4(), cT(), c(upcardValue)));

                    // handle conditions where can't surrender due to table rules
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(-1), false, c6(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(0), false, c6(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(1), false, c6(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(-1), false, c7(), c9(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(0), false, c7(), c9(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(1), false, c7(), c9(), c(upcardValue)));

                    // the random check comes back true

                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, rc(-1), true, c6(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCanSurrender, rc(0), true, c6(), cT(), c(upcardValue))); // deviate
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCanSurrender, rc(1), true, c6(), cT(), c(upcardValue))); // deviate
                    assertEquals(BlackjackPlay.Surrender, compute(tableRulesCanSurrender, rc(-1), true, c7(), c9(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCanSurrender, rc(0), true, c7(), c9(), c(upcardValue))); // deviate
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCanSurrender, rc(1), true, c7(), c9(), c(upcardValue))); // deviate

                    // handle case where can't surrender due to more than 2 cards in hand
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCanSurrender, rc(-1), true, c2(), c4(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCanSurrender, rc(0), true, c2(), c4(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCanSurrender, rc(1), true, c2(), c4(), cT(), c(upcardValue)));

                    // handle conditions where can't surrender due to table rules
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(-1), true, c6(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(0), true, c6(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(1), true, c6(), cT(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(-1), true, c7(), c9(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(0), true, c7(), c9(), c(upcardValue)));
                    assertEquals(BlackjackPlay.Stand, compute(tableRulesCannotSurrender, rc(1), true, c7(), c9(), c(upcardValue)));
                    break;
                case Ace:
